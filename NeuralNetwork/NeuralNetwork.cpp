...
NeuralNetwork::NeuralNetwork(vector<uint> topology, Scalar learningRate)
{
    ...
    for (int i = 0; i < topology.size(); i++)
    {
        ...
        // vector.back() маркер нещодавно доданого елементу
        // coeffRef посилання на значення в цьому місці
        if (i != topology.size() - 1)
        {
...
void NeuralNetwork::updateWeights()
{
    ...
    for (int i = 0; i < topology.size() - 1; i++)
    {
        ...
        /*
        c-й стовпець у матриці ваги представляє з'єднання c-го нейрона в поточному шарі до всіх нейронів попередньому шарі.
        r-й елемент c-го стовпця в матриці ваги представляє з'єднання c-го нейрона в поточному шарі і r-го нейрона в попередньому шарі.
        r-й рядок у матриці ваги представляє з'єднання всіх нейронів попереднього шару з r-им нейроном у поточному шарі.
        c-ий елемент r-го рядка в матриці ваги представляє з'єднання c-го нейрона в попередньому шарі і r-го нейрона в поточному шарі.
         */
...
void NeuralNetwork::train(vector<RowVector*> input_data, vector<RowVector*> output_data)
{
    for (int i = 0; i < input_data.size(); i++)
    {
        ...
        float mse = sqrt((*deltas.back()).dot((*deltas.back())) / (float)deltas.back()->size());
        cout << mse;
        cout << endl;
    }
}
